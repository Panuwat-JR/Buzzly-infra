...
## 🛠️ ขั้นตอนการรัน (Step-by-Step Guide)
...
1.  **Initialize**: ดาวน์โหลด Provider และตั้งค่าเริ่มต้น
    ...
    terraform init
    ```
2.  **Plan**: ตรวจสอบความถูกต้องและดูว่า Terraform จะสร้างทรัพยากรอะไรบ้าง
    ```bash
    terraform plan
    ```
3.  **Apply**: กดยืนยันเพื่อเริ่มสร้างทรัพยากรจริงบน Azure
    ```bash
    terraform apply -auto-approve
    ```
    *(รอประมาณ 1-2 นาทีจนกว่าจะเสร็จสิ้น)*

---

## 🌐 การตรวจสอบผลลัพธ์

เมื่อรัน `terraform apply` สำเร็จ ระบบจะแสดงผลใน Output ดังนี้:
```text
...
## 📝 รายละเอียดทรัพยากรที่ถูกสร้างขึ้น (Architecture)
...
## 🗑️ การลบทรัพยากร (Clean up)

หากต้องการลบทุกอย่างที่สร้างขึ้นเพื่อไม่ให้มีค่าใช้จ่ายค้างในตอนที่ไม่ได้ใช้งานแล้ว:
```bash
...
> [!NOTE]
> ระบบหลังบ้าน (Database & Auth) ในโปรเจกต์นี้เชื่อมต่อกับ **Supabase Cloud** ไว้แล้วใน Code ฝั่ง Web แอปพลิเคชัน ดังนั้นไม่จำเป็นต้องตั้งค่า Database เพิ่มเติมในส่วนของ Infra บน Azure นี้ครับ
